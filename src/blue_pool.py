...
if __name__ == '__main__':
    ...
    flt = create_blue_pool_filter_010(image=img)

    samples = [flt, img]
    sample_images = np.concatenate([image.reshape((1, image.shape[0], image.shape[1], image.shape[2])) for image in samples], axis=0)
